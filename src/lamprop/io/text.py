...
from lamprop.core.utils import is_ortho, to_abaqus_i


def text_output(lam, *, eng=True, mat=True, fea=True) -> list[str]:
...
def _engprop(lam) -> list[str]:
    ...
    lines += [
        f"E_x  = {lam.Ex:.0f} MPa, E_y  = {lam.Ey:.0f} MPa, E_z  = {lam.Ez:.0f} MPa ",
        f"G_xy  = {lam.Gxy:.0f} MPa, G_xz  = {lam.Gxz:.0f} MPa, G_yz  = {lam.Gyz:.0f} MPa ",
        f"nu_xy = {lam.nu_xy:7.5f}",
...
def _fea(lam) -> list[str]:
    ...
    D = to_abaqus_i(lam.C)
    lines.append(f"*MATERIAL,NAME={lam.name}")
    if is_ortho(lam.C):
